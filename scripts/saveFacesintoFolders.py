...
def analyzeKinect():
	...
	D = glob.glob("Real_Scenario" + os.sep + "*.jpg")
	...
	for i, d in enumerate(D):
		rgb = cv2.imread(d, cv2.CV_LOAD_IMAGE_COLOR)
		...
		## AFTER THE FIRST IMAGE = {SECOND ..... LAST IMAGE}
		if flag_first == 0:

			folders = len(folder_position)

			while each_face < all_faces: # for all faces in the image
				
				(x,y,w,h,F) = faces[each_face-1]
				point = x+int(w/2)
				each_folder = 0				
				flag_ok = 0

				while each_folder < folders: # check with all the current folders
					
					if flag_ok == 1: # a face has been inserted in a folder

						break	
					
					if (point > folder_position[each_folder][1] - bandwidth) and (point < folder_position[each_folder][1] + bandwidth):
						dirName = "Persons/Person_" + str(each_folder)						
						crop_img = rgb[y:y+h, x:x+w]
						#cv2.imshow("cropped",crop_img)
						strFile =dirName + os.sep + "image{0:04d}".format(count)
						cv2.imwrite(strFile+'.jpg',crop_img)
						flag_ok = 1
						break
					each_folder = each_folder + 1
									
				if flag_ok == 0: # if this face does not belong to any folder, then create a new folder
					dirName = "Persons/Person_" + str(folder_name)
					os.makedirs(dirName)	
					crop_img = rgb[y:y+h, x:x+w]
					#cv2.imshow("cropped",crop_img)
					strFile =dirName + os.sep + "image{0:04d}".format(count)
					cv2.imwrite(strFile+'.jpg',crop_img)
					folder_position.append([folder_name,point])
					folder_name = folder_name + 1
					break

				each_face = each_face + 1


		## FIRST TIME SEARCH FACES AND CREATE FOLDERS
		if flag_first == 1:			
			while each_face <= all_faces: 
				dirName = "Persons/Person_" + str(folder_name)
				os.makedirs(dirName)
				(x,y,w,h,F) = faces[each_face-1]
				point = x+int(w/2)
		...
		#cv2.imshow("bgr",rgb); cv2.moveWindow('rgb', 0, 0)

		count += 1

	    	if cv2.cv.WaitKey(20) == 27:
			break
